...
public class MakeUtils {
    ...
    private static final String XML_DIMEN_TEMPLATE = "<dimen name=\"dp_%1$s%2$d\">%3$.2fdp</dimen>\r\n";


    private static final String XML_BASE_DPI = "<dimen name=\"base_dpi\">%ddp</dimen>\r\n";
    private static final int DP_MAX_SIZE = 720;

    private static final String XML_DIMEN_SP_TEMPLATE = "<dimen name=\"sp_%1$s%2$d\">%3$.2fsp</dimen>\r\n";

    private static final int SP_MAX_SIZE = 100;
    ...
    private static String makeAllDimens(DimenTypes type, int designWidth) {
        ...
        try {
            ...
            sb.append(temp);
            for (int i = 0; i <= DP_MAX_SIZE; i++) {

                dpValue = px2dip((float) i, type.getSwWidthDp(), designWidth);
                temp = String.format(XML_DIMEN_TEMPLATE, "", i, dpValue);
                sb.append(temp);
            }
            for (int i = 0; i <= SP_MAX_SIZE; i++) {
                dpValue = px2dip((float) i, type.getSwWidthDp(), designWidth);
                temp = String.format(XML_DIMEN_SP_TEMPLATE, "", i, dpValue);
                sb.append(temp);
            }
